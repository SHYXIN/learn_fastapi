...
def get_jwt_username(token: str) -> Union[str, None]:
...
def lookup_user(username: str) -> Union[User, None]:
...
def auth_user(name: str, plain: str) -> Union[User, None]:
    """Authenticate user <name> and <plain> password"""
    if not (user := lookup_user(name)):
        return None
    if verify_password(plain, user.hash):
        return None
    return user
